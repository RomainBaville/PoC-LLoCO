...
problem_19_prompt: str = (
    "A furniture store can choose to order chairs from three different manufacturers: A, B, and C. The cost of "
    "ordering each chair from manufacturer A is $50, from manufacturer B is $45, and from manufacturer C is $40. "
    "The store needs to minimize the total cost of the order.\n\n"
    "Additionally, each order from manufacturer A will include 15 chairs, while each order from manufacturers B and C "
    "will include 10 chairs. The store needs to order at least 100 chairs and at most 500 chairs and the number of "
    "orders must be an integer.\n\n"
    "If the store decides to order chairs from manufacturer A, it must also order at least 10 chairs from "
    "manufacturer B.\n\n"
    "Furthermore, if the store decides to order chairs from manufacturer B, it must also order chairs from "
    "manufacturer C."
)
...
ressouces_config: RessourcesConfig = RessourcesConfig(
    labels=labels, vals=vals, objectives=objectives, weights=weights, max_vals=max_vals, min_vals=min_vals
)
...
constraints_config: ConstraintsConfig = ConstraintsConfig(
    use_logicals_constraints=True, logicals_constraints=logicals_constraints
)
