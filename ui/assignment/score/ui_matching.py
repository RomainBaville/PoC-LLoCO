...
def matching_strategy( session_state: SessionStateProxy ) -> None:
    ...
    Args:
        session_state (SessionStateProxy): The session state.
    """
    session_state.matching_objective = st.selectbox(
        " Set the objective for the matching", Objective, disabled=session_state.lock_strategy
    )
    ...
    use_matching_weights: bool = st.checkbox(
        "Is there varibales to match with weights", disabled=session_state.lock_strategy
    )
    if use_matching_weights:
        matching_labels: tuple[ str, ...] = tuple(
            st.multiselect(
                "Select the variables with a weight",
                session_state.matching_labels,
                disabled=session_state.lock_strategy
            )
        )
        if len( matching_labels ) > 0:
            for matching_label in matching_labels:
                session_state.matching_weights[ matching_label ] = st.number_input(
                    f"Weight for { matching_label }", value=1., disabled=session_state.lock_strategy
                )
        ...
        with matching_extrema_col:
            ...
            if use_matching_extrema_vals:
                variables_labels: tuple[ str, ...] = tuple(
                    st.multiselect(
                        f"Select variables constrained by a { extrema[ id ] } value",
                        session_state.matching_labels,
                        disabled=session_state.lock_constraints
                    )
                )
