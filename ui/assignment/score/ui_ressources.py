...
def map_ressources( state ):
    state.ressources_labels = tuple( st.multiselect( "Select columns identifying your ressources", state.left_cols, ) )
...
def ressources_constraints( state ):
    ...
    constraints_extrema_vals: list[ Optional[ dict[ tuple[ str, ...], float ] ] ]  = [ None, None ]
    for id, constraints_extrema_col in enumerate( constraints_extrema_cols ):
        with constraints_extrema_col:
            use_constraints_extrema_vals: bool = st.checkbox( f"Is there ressources constrained by a { extrema[ id ] } value ?" )
            if use_constraints_extrema_vals:
                constraints_labels: tuple[ str, ...] = tuple( st.multiselect( f"Select all variables used as { extrema[ id ] } constraint", state.right_cols ) )
                if len( constraints_labels ) > 0:
                    constraints_ressources_labels: dict[ str, list[ str ] ] = {}
                    for constraint_label in constraints_labels:
                        constraints_ressources_labels[ constraint_label ] = st.multiselect( f"Select ressources constrainning by { constraint_label }", state.ressources_labels )
    ...
    constraints_extrema_global_vals: list[ Optional[ dict[ tuple[ str, ...], float ] ] ] = [ None, None ]
    if len( state.right_labels ) > 1:
        ...
        for id, constraints_extrema_global_col in enumerate( constraints_extrema_global_cols ):
            with constraints_extrema_global_col :
                use_constraints_extrema_global_vals: bool = st.checkbox( f"Is there group of ressources constrained with a { extrema[ id ] } value for all { state.right_entities_type }" )
                if use_constraints_extrema_global_vals:
                    constraints_extrema_global_vals[ id ] = {}
                    nb_constrained_groups: int = st.number_input( "How many group of ressources are constrained", value = 1 )
                    for _ in range( nb_constrained_groups ):
                        constrained_ressources: tuple[ str, ...] = tuple( st.multiselect( f"Select the ressources constrained by the same { extrema[ id ] } value for all { state.right_entities_type }", state.ressources_labels ) )
                        if len( constrained_ressources ) > 0:
                            constraint_val: float = st.number_input( f"Set the { extrema[ id ] } value constrainning the { constrained_ressources }", value = 1. )
