...
def quantities_constraints( state ) -> None:
        ...
        for id, extrema_side_entities_col in enumerate( extrema_side_entities_cols ):  # max and min columns
            with extrema_side_entities_col:
                use_extrema_side_entities: bool = st.checkbox(
                    f"Is there a { extrema[ id ] } number of { entities_types[ 1 - side ] } that can be assigned to one { entities_types[ side ] } ?"
                )
    ...
    # -----------------------------
    state.multiple_same_assignment = st.checkbox(
        f"{ state.left_entities_type } can be assigned multiple time to the same { state.right_entities_type } ?"
    )
    ...
    if state.multiple_same_assignment:
        ...
        for id in range( 2 ):  # max then min
            use_extrema_same_assignments: bool = st.checkbox(
                f"Is there { state.left_entities_type } with a { extrema[ id ] } number of assignment to the same { state.right_entities_type } ?"
            )
            if use_extrema_same_assignments:
                extrema_same_assignments[ id ] = {}
                left_labels_constrainted: tuple[ str, ...] = tuple(
                    st.multiselect( f"Select all the { state.left_entities_type } with a constraint", state.left_cols )
                )
